...
class DrawGeoJson(object):
    ...
    def adjust_poly_dictionary(self):
        ...
        #pp.pprint(self.adjusted_poly_dict)
    #Method to print borders of the selected country and rectangular box 
    def drawpoly(self,position):
        black=(0,0,0)
        
        for k,v in self.adjusted_poly_dict.items():
            xlong = []
            ylat = []
            newlist=[]
            for coords in v:
                for h in coords:
                   newlist.append(h)
                if point_inside_polygon(position[0],position[1],newlist):
                    #for i in newlist:
                        #print(i)
                        #For borders
                    pygame.draw.polygon(self.screen,black,newlist,4)
                    # to print the name of the country
                    pygame.init()
                    myfont = pygame.font.SysFont(None,56)
                    text = myfont.render(k,True,black)
                    screen.blit(text,(800,100))
                    for poly in range(len(newlist)):
                        xlong.append(newlist[poly][0])
                        ylat.append(newlist[poly][1])
                    min_country_long = min(xlong)
                    max_country_long = max(xlong)
                    min_country_lat = min(ylat)
                    max_country_lat = max(ylat)
                    p,q = min_country_long,min_country_lat
                    r,s = min_country_long,max_country_lat
                    t,u = max_country_long,max_country_lat
                    v,w = max_country_long,min_country_lat
                    

                    pygame.draw.lines(screen,(255,0,0),True,[(p,q), (r,s), (t,u), (v,w)],4)
...
if __name__ == '__main__':
    ...
    df.add_polygons(['Spain','Finland','Belgium','Algeria','Ireland','Iran','southafrica','Germany','Egypt','Morocco','India'])
